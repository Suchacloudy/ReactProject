...
function logWorkboxResult(strategy, buildResult, viteOptions) {
  ...
  if (logLevel === "info") {
    console.info([
      "",
      `${(0, import_chalk.cyan)(`PWA v${version}`)}`,
      `mode      ${(0, import_chalk.magenta)(strategy)}`,
      `precache  ${(0, import_chalk.green)(`${count} entries`)} ${(0, import_chalk.dim)(`(${(size / 1024).toFixed(2)} KiB)`)}`,
      "files generated",
      ...filePaths.map((p) => `  ${(0, import_chalk.dim)((0, import_path.relative)(root, p))}`)
    ].join("\n"));
  }
  warnings && warnings.length > 0 && console.warn((0, import_chalk.yellow)([
    "warnings",
    ...warnings.map((w) => `  ${w}`),
    ""
  ].join("\n")));
...
async function generateServiceWorker(options, viteOptions) {
  const buildResult = await (0, import_workbox_build.generateSW)(options.workbox);
...
function VitePWA(userOptions = {}) {
  ...
  function _generateBundle(bundle = {}) {
    ...
    if (options.injectRegister === "script" && !(0, import_fs4.existsSync)((0, import_path5.resolve)(viteConfig.publicDir, FILE_SW_REGISTER))) {
